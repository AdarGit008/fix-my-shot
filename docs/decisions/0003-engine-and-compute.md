...
- **Runtime:** MuJoCo via its official WebAssembly build, in-browser, primitive (capsule/link) rendering — realism over aesthetics. Deterministic.
- **Offline:** MuJoCo MJX (JAX, differentiable, GPU/TPU) for starting-state generation and for the gradients that feed the leverage layer (ADR-0004). MJX is ~10× slower per single scene → offline only, never on the interactive path.
